...
#'
#'@details Takes an \code{\link{MAF}} object as an input and plots it as a matrix. Any desired
#'  clincal features can be added at the bottom of the oncoplot by providing
#'  \code{clinicalFeatures}. Oncoplot can be sorted either by mutations or by clinicalFeatures using
#'  arguments \code{sortByMutation} and \code{sortByAnnotation} respectively.
#'
#'  By setting `pathways` argument either `sigpw` or `smgbp` - cohort can be summarized by altered
#'  pathways. pathways argument also accepts a custom pathway list in the form of a two column tsv
#'  file or a data.frame containing gene names and their corresponding pathway.
#'
#'@references Bailey, Matthew H et al. “Comprehensive Characterization of Cancer Driver Genes and
#'  Mutations.” Cell vol. 173,2 (2018): 371-385.e18. doi:10.1016/j.cell.2018.02.060 Sanchez-Vega,
#'  Francisco et al. “Oncogenic Signaling Pathways in The Cancer Genome Atlas.” Cell vol. 173,2
#'  (2018): 321-337.e10. doi:10.1016/j.cell.2018.03.035
#'
#'@param maf an \code{\link{MAF}} object generated by \code{\link{read.maf}}
#'@param top how many top genes to be drawn. defaults to 20.
#'@param minMut draw all genes with `min` number of mutations. Can be an integer or fraction (of
#'  samples mutated), Default NULL
#'@param genes Just draw oncoplot for these genes. Default NULL.
#'@param altered Default FALSE. Chooses top genes based on muatation status. If \code{TRUE} chooses
#'  top genes based alterations (CNV or mutation).
...
#'@param leftBarData Data for leftside barplot. Must be a data.frame with two columns containing
#'  gene names and values. Default `NULL`
...
#'@param topBarData Default `NULL` which draws absolute number of mutation load for each sample. Can
#'  be overridden by choosing one clinical indicator(Numeric) or by providing a two column
#'  data.frame containing sample names and values for each sample. This option is applicable when
#'  only `drawColBar` is TRUE.
...
#'@param rightBarData Data for rightside barplot. Must be a data.frame with two columns containing
#'  to gene names and values. Default `NULL` which draws distribution by variant classification.
#'  This option is applicable when only `drawRowBar` is TRUE.
...
#'@param clinicalFeatures columns names from `clinical.data` slot of \code{MAF} to be drawn in the
#'  plot. Default NULL.
#'@param annotationColor  Custom colors to use for `clinicalFeatures`. Must be a named list
#'  containing a named vector of colors. Default NULL. See example for more info.
#'@param annotationDat If MAF file was read without clinical data, provide a custom
#'  \code{data.frame} with a column \code{Tumor_Sample_Barcode} containing sample names along with
#'  rest of columns with annotations. You can specify which columns to be drawn using
#'  `clinicalFeatures` argument.
#'@param pathways Default `NULL`. Can be `sigpw`, `smgbp`, or a two column data.frame/tsv-file with
#'  genes and corresponding pathway mappings.`
#'@param topPathways Top most altered pathways to draw. Default 3. Mutually exclusive with
#'  `selectedPathways`
#'@param path_order Default `NULL` Manually specify the order of pathways
#'@param selectedPathways Manually provide the subset of pathway names to be selected from
#'  `pathways`. Default NULL. In case `pathways` is `auto` draws top 3 altered pathways.
...
#'@param titv_col named vector of colors for each transition and transversion classes. Should be of
#'  length six with the names "C>T" "C>G" "C>A" "T>A" "T>C" "T>G".  Default NULL.
#'@param showTumorSampleBarcodes logical to include sample names.
#'@param tsbToPIDs Custom names for Tumor_Sample_Barcodes. Can be a column name in clinicaldata or a
#'  2 column data.frame of Tumor_Sample_Barcodes to patient ID mappings. Applicable only when
#'  `showTumorSampleBarcodes = TRUE`. Default NULL.
...
#'@param lengend_txt_replace Named vector like c(Frame_Shift_Ins='Amplification',
#'  In_Frame_Ins='Gain', In_Frame_Del='Loss', Frame_Shift_Del='Deletion'). replace standard item
#'  like "Frame_Shift_Ins" to custom label like 'Amplification' in figure legend. Do not affect data
#'  presentation. Can be used to display CNA (copy number alteration) data of genes instead of
#'  mutations of genes (SNV) of oncoplot, for both plots differ at figure legends only, CNA prefers
#'  "Loss", "Deletion", "Gain", etc in legend, while SNV prefers "Frame_Shift_Ins",
#'  "Frame_Shift_Del", etc in legend.
#'@param sortByAnnotation logical sort oncomatrix (samples) by provided `clinicalFeatures`. Sorts
#'  based on first `clinicalFeatures`.  Defaults to FALSE. column-sort
#'@param groupAnnotationBySize Further group `sortByAnnotation` orders by their size.  Defaults to
#'  TRUE. Largest groups comes first.
#'@param annotationOrder Manually specify order for annotations. Works only for first
#'  `clinicalFeatures`. Default NULL.
#'@param sortByMutation Force sort matrix according mutations. Helpful in case of MAF was read along
#'  with copy number data. Default FALSE.
#'@param keepGeneOrder logical whether to keep order of given genes. Default FALSE, order according
#'  to mutation frequency
...
#'@param additionalFeature a vector of length two indicating column name in the MAF and the factor
#'  level to be highlighted. Provide a list of values for highlighting more than one features
...
#'  for better visualization. Default \code{FALSE}.
#'@param fill Logical. If \code{TRUE} draws genes and samples as blank grids even when they are not
#'  altered.
#'@param cohortSize Number of sequenced samples in the cohort. Default all samples from Cohort. You
#'  can manually specify the cohort size. Default \code{NULL}
#'@param colors named vector of colors for each Variant_Classification.
#'@param cBioPortal Adds annotations similar to cBioPortals MutationMapper and collapse Variants
#'  into Truncating and rest.
...
#'@param numericAnnoCol color palette used for numeric annotations. Default 'YlOrBr' from
#'  RColorBrewer
...
oncoplot = oncoplot = function(maf, top = 20, minMut = NULL, genes = NULL, altered = FALSE,
                               ...
                               anno_height = 1, legend_height = 4, lengend_txt_replace = NULL,
